...
// Zero-width: Mn/Me general categories, plus format characters that terminals treat
// as non-advancing (ZWNJ/ZWJ, variation selectors, Hangul fillers).
constexpr Range kZeroWidth[] = {
    ...
    {0x0829, 0x082D}, {0x0859, 0x085B}, {0x0898, 0x089F}, {0x08CA, 0x08E1},
    ...
    {0x0B4D, 0x0B4D}, {0x0B55, 0x0B56}, {0x0B62, 0x0B63}, {0x0B82, 0x0B82},
    {0x0BC0, 0x0BC0}, {0x0BCD, 0x0BCD}, {0x0C00, 0x0C00}, {0x0C04, 0x0C04},
    {0x0C3C, 0x0C3C}, {0x0C3E, 0x0C40}, {0x0C46, 0x0C48}, {0x0C4A, 0x0C4D},
    ...
    {0x10A38, 0x10A3F}, {0x10AE5, 0x10AE6}, {0x10D24, 0x10D27}, {0x10EAB, 0x10EAC},
    {0x10F46, 0x10F50}, {0x11001, 0x11001}, {0x11038, 0x11046}, {0x11070, 0x11070},
    ...
    {0x11C30, 0x11C36}, {0x11C38, 0x11C3D}, {0x11C92, 0x11CA7}, {0x11CAA, 0x11CB0},
    ...
    {0x11D95, 0x11D95}, {0x11D97, 0x11D97}, {0x11EF3, 0x11EF4}, {0x13440, 0x13440},
...
// East Asian Wide and Fullwidth, plus the emoji ranges terminals render double-width.
...
bool InTable(const Range (&table)[N], char32_t c) {
  std::size_t low = 0;
  std::size_t high = N;
  while (low < high) {
...
bool IsZeroWidth(char32_t code_point) {
  if (code_point == 0) return true;
  if (code_point < 0x00AD) return false;
  return InTable(kZeroWidth, code_point);
}

int CharWidth(char32_t code_point) {
  if (code_point < 0x20 || (code_point >= 0x7F && code_point < 0xA0)) return 0;
  if (code_point < 0xA0) return 1;
  if (IsZeroWidth(code_point)) return 0;
  if (InTable(kWide, code_point)) return 2;
  return 1;
}
